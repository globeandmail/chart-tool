...
var Settings = function Settings() {
  ...
  this.hasHours = false;
  this.social = social;
  this.seriesHighlight = function () {
    return (this$1.data.seriesAmount && this$1.data.seriesAmount <= 1) ? 1 : 0;
  };
  this.baseClass = function () { return ((this$1.prefix) + "chart"); };
  this.customClass = '';
...
var Recipe = (function (Settings$$1) {
  ...
  if ( Settings$$1 ) Recipe.__proto__ = Settings$$1;
  Recipe.prototype = Object.create( Settings$$1 && Settings$$1.prototype );
...
function setScaleType(type) {
  switch (type) {
    ...
    case 'identity': return identity$1();
    case 'pow': return pow();
    case 'sqrt': return sqrt();
    case 'log': return log();
    case 'quantize': return quantize$1();
    case 'quantile': return quantile$$1();
    case 'threshold': return threshold$1();
...
function rescaleNumerical(scale) {

  // rescales the 'top' end of the domain
  var ticks$$1 = scale.ticks(10).slice(),
    tickIncr = Math.abs(ticks$$1[ticks$$1.length - 1]) - Math.abs(ticks$$1[ticks$$1.length - 2]);

  var newMax = ticks$$1[ticks$$1.length - 1] + tickIncr;
...
function appendYAxis(axisGroup, obj, scale, axis) {
...
function discreteAxis(axisNode, scale, axis, axisSettings, dimensions) {
  ...
  var firstXPos = getTranslate(axisNode.select('.tick').node())[0] * -1;
...
function dropTicks(selection$$1, opts) {
  ...
  for (var j = from; j < to; j++) {
    ...
    var c = selection$$1._groups[j]; // current selection
    var n = selection$$1._groups[j + 1]; // next selection
...
function dropOversetTicks(axisNode, tickWidth) {
  ...
  if (tickArr.length) {
    ...
    var firstTickOffset = getTranslate(select(tickArr[0])
      .node())[0];
  ...
  if (!isServer) {
    ...
    var ChartTool = (function ChartTool() {
      ...
      function updateChart(id, obj) {
        var container = "." + (s.baseClass()) + "[data-chartid=" + (s.prefix) + id + "]";
      ...
      function createLoop(charts) {
        ...
        for (var i = 0; i < chartList.length; i++) {
          ...
          var container = "." + (s.baseClass()) + "[data-chartid=" + (chartList[i]) + "]";
